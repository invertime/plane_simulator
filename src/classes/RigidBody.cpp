...
void RigidBody::add_force_at_point(const Vector3f& force, const Vector3f& point){
...
void RigidBody::update(float dt){
  ...
  if (applyGravity) acceleration.y -= 9.81f;
  velocity += acceleration*dt;
  position += velocity*dt;

  // integrate orientation
  angularVelocity += inverseInertia * (m_torque - crossProduct(angularVelocity, inertiaTensor * angularVelocity) * dt);
  orientation += (orientation * Quaternion(0.0f, angularVelocity)) * (0.5f * dt);
};
